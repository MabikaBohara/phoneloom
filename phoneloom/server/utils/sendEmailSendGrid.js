...
const sendEmailSendGrid = async (to, subject, text, html = null) => {
  try {
    ...
    const msg = {
      to: to,
      from: {
        email: process.env.SENDGRID_FROM_EMAIL || 'noreply@phoneloom.com',
        name: 'PhoneLoom Support'
      },
      subject: subject,
      text: text,
      html: html || text,
...
const sendPasswordResetEmailSendGrid = async (email, resetURL, userName) => {
...
PhoneLoom Team
  ...
  return await sendEmailSendGrid(email, subject, text, html);
};

module.exports = { sendEmailSendGrid, sendPasswordResetEmailSendGrid };
